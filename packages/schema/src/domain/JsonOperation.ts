...
function createDeepSchema(expression: string[], schema: JsonSchema, model: any, required: boolean) {
  const [key, ...rest] = expression;

  if (rest.length) {
    const deepSchema = new JsonSchema({
      type: "object"
    });

    createDeepSchema(rest, deepSchema, model, required);

    schema.addProperties(key, deepSchema);
  } else {
    schema.addProperties(key, model);
  }
